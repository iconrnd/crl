...
class EnsembleRSSM(common.Module):
  ...
  def __init__(
      ...
      act='elu', norm='none', std_act='softplus', min_std=0.1, z_out=False):
    super().__init__()
...
@tf.function
def dimeff_reg(mlt=0.01):
    def dimeff_reg(weights):
        means = tf.math.reduce_mean(weights, axis=0)
        weights = tf.math.subtract(weights, means)
        s, _, _ = tf.linalg.svd(tf.matmul(tf.transpose(weights), weights), full_matrices=True)

        x = tf.math.divide(s, tf.reduce_sum(s))

        return mlt * tf.math.exp(tf.math.reduce_sum(-tf.math.multiply_no_nan(x, tf.math.log(x))))
    return dimeff_reg


class MLP(common.Module):

  def __init__(self, shape, layers, units, act='elu', norm='none', dynamic=False, z_out=False, **out):
  ...
  def __call__(self, features):
    ...
    for index in range(self._layers):
      if index == 3:
          x = self.get(f'dense{index}', tfkl.Dense, self._units, kernel_regularizer=dimeff_reg)(x)
      else:
          x = self.get(f'dense{index}', tfkl.Dense, self._units)(x)
      if index == 3 and self._z_out:
        #print(f'MPL z: {1*(x.numpy() > 0)}, {index}')
        # TODO actually return just activations, so no need to 
        # convert to numpy. tf.summary will store x and
        # z will be computed outside during analysis
        x_out = x
      x = self.get(f'norm{index}', NormLayer, self._norm)(x)
      x = self._act(x)
    x = x.reshape(features.shape[:-1] + [x.shape[-1]])
    if self._z_out:
        return self.get('out', DistLayer, self._shape, **self._out)(x), x_out
    else:
