...
# vi0: Used to sample from a distribution passed in init
class SampleDist:
...
class OneHotDist(tfd.OneHotCategorical):
  ...
  def sample(self, sample_shape=(), seed=None):
    ...
    # vi0: This has zero value but non-zero gradient: 
    # added for differentiable predictions not affecting the values
    sample += tf.cast(probs - tf.stop_gradient(probs), self._sample_dtype)
    return sample

  # vi0: Extends tensor to the shape passed in arg
