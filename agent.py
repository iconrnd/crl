...
class Agent(common.Module):
  ...
  def policy(self, obs, state=None, mode='train'):
    ...
    if mode == 'eval':
      # vi0: here actor returns tuple with z
      # vi0: TODO introduce conditioning on config 
      actor, z = self._task_behavior.actor(feat)
      action = actor.mode()
      noise = self.config.eval_noise
      # print(z)
    elif mode == 'explore':
      actor, z = self._expl_behavior.actor(feat)
      # vi0: here actor returns tuple with z
      # vi0: TODO introduce conditioning on config
      action = actor.sample()
    ...
    state = (latent, action)
    return outputs, state, z


  @tf.function
  def train(self, data, state=None):
    metrics = {}
    # Here one can input entropy variable harvest
...
class WorldModel(common.Module):
  ...
  def imagine(self, policy, start, is_terminal, horizon):
    ...
    # vi0: here policy is actor returning tuple with z
    # vi0: TODO introduce conditioning on config
    start['action'] = tf.zeros_like(policy(start['feat'])[0].mode())
    seq = {k: [v] for k, v in start.items()}
    for _ in range(horizon):
      # vi0: here policy is actor returning tuple with z
      # vi0: TODO introduce conditioning on config
      action = policy(tf.stop_gradient(seq['feat'][-1]))[0].sample()
...
class ActorCritic(common.Module):
  ...
  def actor_loss(self, seq, target):
    ...
    if self.config.actor_grad == 'dynamics':
      ...
      # vi0: here policy is actor returning tuple with z
      # vi0: TODO introduce conditioning on config
      objective = policy[0].log_prob(action) * advantage
